...
export function validateProduct(product) {
  ...
  if (product.width < MIN_DIMENSION || product.width > MAX_DIMENSION) {
    errors.push(`Ширина должна быть от ${MIN_DIMENSION} до ${MAX_DIMENSION} см`)
  }
  if (product.height < MIN_DIMENSION || product.height > MAX_DIMENSION) {
    errors.push(`Высота должна быть от ${MIN_DIMENSION} до ${MAX_DIMENSION} см`)
  }
  return errors
}
